Shader "Gooest/LBM/step" {
	...
	float4 step1(v2f_img IN) : SV_Target {
		...
		if(_T > 0.) {
			float2 uv = IN.uv;
			float f1 = tex2D(_Diffuse1, uv - float2(_InverseSize.x,   0)).r;
			float f2 = tex2D(_Diffuse1, uv - float2(_InverseSize.x, _InverseSize.y)).g;
			float f3 = tex2D(_Diffuse1, uv - float2(  0, _InverseSize.y)).b;
	...
	float4 step2(v2f_img IN) : SV_Target {
		...
		if(_T > 0.) {
			float2 uv = IN.uv;
			float f4 = tex2D(_Diffuse2, uv - float2(-_InverseSize.x,  _InverseSize.y)).r;
			float f5 = tex2D(_Diffuse2, uv - float2(-_InverseSize.x,    0)).g;
			float f6 = tex2D(_Diffuse2, uv - float2(-_InverseSize.x, -_InverseSize.y)).b;
	...
	float4 step3(v2f_img IN) : SV_Target {
		...
		if(_T > 0.) {
			float2 uv = IN.uv;
			float f7 = tex2D(_Diffuse3, uv - float2(   0, -_InverseSize.y)).r;
			float f8 = tex2D(_Diffuse3, uv - float2( _InverseSize.x, -_InverseSize.y)).g;
